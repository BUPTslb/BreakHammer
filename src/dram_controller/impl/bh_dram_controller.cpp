...
class BHDRAMController final : public IBHDRAMController, public Implementation {
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      // BreakHammer
      read_cycles = m_dram->m_timing_vals("nRAS") + m_dram->m_timing_vals("nRTP") + m_dram->m_timing_vals("nRP");
    ...
    bool send(Request& req) override {
      ...
      // Skip void attacker requests
      if (req.spec_type == Request::SpecType::CacheOnly) {
        req.depart = m_clk + read_cycles;
