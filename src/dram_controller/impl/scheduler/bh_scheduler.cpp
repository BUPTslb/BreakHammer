...
#ifndef HAMMEREVO_BH_QOS_DATAPATH_SHA256
#define HAMMEREVO_BH_QOS_DATAPATH_SHA256 "unavailable"
#endif
...
class BHScheduler : public IBHScheduler, public Implementation {
  ...
  public:
    void init() override {
      ...
      std::cout << "scheduler_impl: BHScheduler" << std::endl;
      std::cout << "scheduler_qos_datapath_sha256: "
                << HAMMEREVO_BH_QOS_DATAPATH_SHA256 << std::endl;
      ...
      std::cout << "scheduler_qos_max_priority_disadvantage_cycles: "
                << m_qos_max_starvation_cycles << std::endl;
      // A same-bank launch guard intentionally gives up global work
      // conservation while a protected request is pending. Other banks remain
      // eligible, and an aged background request regains ordinary priority.
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      if (m_bank_slot_count <= 0 || m_bank_slot_count > 64) {
        throw std::runtime_error(
          "BHScheduler same-bank launch guard supports at most 64 bank slots");
      }
    ...
    ReqBuffer::iterator compare(ReqBuffer::iterator req1, ReqBuffer::iterator req2) override {
      // Work conservation precedes service class: never idle a command bus
      // merely because an unready normal request exists while a background
      // command is ready.  QoS orders candidates only within the same
      // readiness class.
    ...
    ReqBuffer::iterator get_best_request(
      ...
      for (auto next = buffer.begin(); next != buffer.end(); next++) {
        if (!is_active_buffer
          ...
          guarded_ready_background =
            guarded_ready_background || static_cast<bool>(next->scratch0);
          continue;
        }
      ...
      // Calls for the active buffer bypass launch_guard_blocks(), so a
      // background request that has already issued ACT always remains
      // drainable. An untouched background request may launch on an
      // uncontended bank, or after its finite aging bound.
