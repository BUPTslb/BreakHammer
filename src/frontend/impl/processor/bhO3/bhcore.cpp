...
void BHO3Core::tick() {
  ...
  if (m_load_addr != -1) {
    ...
    Request load_request(m_load_addr, Request::Type::Read, m_spec_type, m_id, m_callback);
  ...
  if (m_writeback_addr != -1) {
    Request writeback_request(m_writeback_addr, Request::Type::Write, m_spec_type, m_id, m_callback);
